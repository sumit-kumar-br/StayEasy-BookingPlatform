...
{
    /// <summary>
    /// Consumes integration events in ManagerBookingCancelledConsumer.
    /// </summary>
    public class ManagerBookingCancelledConsumer : IConsumer<ManagerBookingCancelledEvent>
    {
        private readonly INotificationLogService _notificationService;

        public ManagerBookingCancelledConsumer(INotificationLogService notificationService)
        {
        ...
        /// </summary>
        public async Task Consume(ConsumeContext<ManagerBookingCancelledEvent> context)
        {
            var msg = context.Message;

            var subject = "Booking Cancelled";
            var body = BuildManagerBookingCancelledEmail(msg);

            await _notificationService.HandleEmailAsync(
                msg.EventId,
                nameof(ManagerBookingCancelledEvent),
                msg.ManagerId,
                msg.Email,
                subject,
                body,
        ...
        private static string BuildManagerBookingCancelledEmail(ManagerBookingCancelledEvent msg)
        {
            ...
            var cancelledBy = WebUtility.HtmlEncode(msg.CancelledBy);
          ...
          <table role='presentation' width='640' cellspacing='0' cellpadding='0' style='width:100%;max-width:640px;background:#ffffff;border:1px solid #dce5f0;border-radius:14px;overflow:hidden;'>
            <tr>
              <td style='padding:22px 24px;background:linear-gradient(120deg,#0f766e 0%,#115e59 100%);color:#ffffff;'>
                ...
                <h1 style='margin:10px 0 4px;font-size:26px;line-height:1.2;'>Booking cancelled</h1>
                <p style='margin:0;font-size:15px;opacity:0.95;'>A booking linked to your property has been cancelled.</p>
              </td>
            </tr>
            <tr>
              <td style='padding:24px;'>
                <table role='presentation' width='100%' cellspacing='0' cellpadding='0' style='border:1px solid #e7edf5;border-radius:10px;background:#f8fbff;'>
                  ...
                  <tr>
                    <td style='padding:0 18px 8px;'>
                      ...
                      <p style='margin:8px 0 0;font-size:15px;color:#334e68;'><strong>Cancelled By:</strong> {cancelledBy}</p>
                    </td>
                  </tr>
                  <tr>
                ...
                <p style='margin:18px 2px 0;font-size:14px;color:#48627a;line-height:1.6;'>
                  This is a cancellation copy for your records and operations.
                </p>
              </td>
            </tr>
            <tr>
