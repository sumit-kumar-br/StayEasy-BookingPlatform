...
{
    /// <summary>
    /// Consumes integration events in ManagerBookingConfirmedConsumer.
    /// </summary>
    public class ManagerBookingConfirmedConsumer : IConsumer<ManagerBookingConfirmedEvent>
    {
        private readonly INotificationLogService _notificationService;

        public ManagerBookingConfirmedConsumer(INotificationLogService notificationService)
        {
        ...
        /// </summary>
        public async Task Consume(ConsumeContext<ManagerBookingConfirmedEvent> context)
        {
            var msg = context.Message;

            var subject = "Booking Confirmed";
            var body = BuildManagerBookingConfirmedEmail(msg);

            await _notificationService.HandleEmailAsync(
                msg.EventId,
                nameof(ManagerBookingConfirmedEvent),
                msg.ManagerId,
                msg.Email,
                subject,
                body,
        ...
        private static string BuildManagerBookingConfirmedEmail(ManagerBookingConfirmedEvent msg)
        {
          ...
          <table role='presentation' width='640' cellspacing='0' cellpadding='0' style='width:100%;max-width:640px;background:#ffffff;border:1px solid #dce5f0;border-radius:14px;overflow:hidden;'>
            <tr>
              <td style='padding:22px 24px;background:linear-gradient(120deg,#0f766e 0%,#115e59 100%);color:#ffffff;'>
                ...
                <h1 style='margin:10px 0 4px;font-size:26px;line-height:1.2;'>Booking confirmed</h1>
                <p style='margin:0;font-size:15px;opacity:0.95;'>This booking has been confirmed for your property.</p>
              </td>
            </tr>
            <tr>
              <td style='padding:24px;'>
                ...
                <p style='margin:18px 2px 0;font-size:14px;color:#48627a;line-height:1.6;'>
                  This is a confirmation copy for your records and operations.
                </p>
              </td>
            </tr>
            <tr>
